...
describe("CoursesController e2e tests", () => {
  ...
  const dataSurceTest: DataSourceOptions = {
    type: "postgres",
  ...
  beforeEach(async () => {
    const dataSource = await new DataSource(dataSurceTest).initialize();
    const reponsitory = dataSource.getRepository(Course);
    courses = await reponsitory.find();
